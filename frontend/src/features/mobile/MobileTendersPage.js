  ...
  const loadTendersData = async () => {
    try {
      ...
      if (activeTab === 'my-bids') {
        ...
        const response = await fetch(`${process.env.REACT_APP_BACKEND_URL || 'https://admin-nav-control.preview.emergentagent.com/api'}/api/tenders/buyer/${user.id}`);
        if (response.ok) {
        ...
        const response = await fetch(`${process.env.REACT_APP_BACKEND_URL || 'https://admin-nav-control.preview.emergentagent.com/api'}/api/tenders/seller/${user.id}/overview`);
        if (response.ok) {
          const data = await response.json();
...
function SellingCard({ listingOverview, onManageTenders }) {
  ...
  const handleAcceptTender = async (tenderId, tenderAmount) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL || 'https://admin-nav-control.preview.emergentagent.com/api'}/api/tenders/${tenderId}/accept`, {
        method: 'PUT',
  ...
  const handleRejectTender = async (tenderId, tenderAmount) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL || 'https://admin-nav-control.preview.emergentagent.com/api'}/api/tenders/${tenderId}/reject`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ seller_id: listingOverview.listing?.seller_id })
      });
