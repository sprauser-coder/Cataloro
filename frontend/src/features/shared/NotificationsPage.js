...
function NotificationsPage() {
  ...
  // Smart notification click handler (same logic as ModernHeader and NotificationsCenterPage)
  const handleNotificationClick = async (notification) => {
    try {
      // Mark notification as read first if not already read
      ...
      switch (notification.type) {
        ...
        case 'user_registration':
          ...
          if (user?.role === 'admin' || user?.is_admin) {
            ...
            // For non-admin users, stay on notifications page
            console.log('User registration notification is for admin users only');
          }
          break;
          
        case 'system':
        case 'announcement':
          // Stay on notifications page for full details (keep existing)
          console.log('System notification details available here');
          break;
          
        default:
          // For unknown notification types, check if action_url exists (fallback to old behavior)
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex items-center space-x-4">
              ...
              <button
                ...
                disabled={loading}
                className="p-3 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
