...
function SimpleLoginPage() {
  ...
  useEffect(() => {
    const fetchLogo = async () => {
      try {
        const response = await fetch(`${process.env.REACT_APP_BACKEND_URL || 'https://mobileui-sync.preview.emergentagent.com/api'}/admin/logo`);
        if (response.ok) {
  ...
  const handleLoginSubmit = async (e) => {
    ...
    try {
      ...
      const backendUrl = process.env.REACT_APP_BACKEND_URL || 'https://mobileui-sync.preview.emergentagent.com/api';
  ...
  const handleRegisterSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL || 'https://mobileui-sync.preview.emergentagent.com/api'}/api/auth/register`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
