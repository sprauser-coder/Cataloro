            ...
            {(permissions.ui.showAdminPanelLink) && (product.ceramic_weight || product.pt_ppm || product.pd_ppm || product.rh_ppm) && (
              <div className="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-xl p-6 border border-blue-200 dark:border-blue-800">
                <div className="flex items-center space-x-2 mb-4">
                  ...
                  <h3 className="text-lg font-semibold text-blue-900 dark:text-blue-100">Catalyst Database Fields</h3>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {product.ceramic_weight && (
                    <div className="text-center p-3 bg-white dark:bg-gray-800 rounded-lg border border-blue-100 dark:border-blue-800">
                      ...
                      <div className="text-lg font-bold text-gray-900 dark:text-white">{product.ceramic_weight}g</div>
                    </div>
                  )}
                  
                  {product.pt_ppm !== undefined && (
                    <div className="text-center p-3 bg-white dark:bg-gray-800 rounded-lg border border-blue-100 dark:border-blue-800">
                      <div className="text-xs text-blue-600 dark:text-blue-400 uppercase font-medium mb-1">Pt ppm</div>
                      <div className="text-lg font-bold text-gray-900 dark:text-white">{product.pt_ppm}</div>
                    </div>
                  )}
                  
                  {product.pd_ppm !== undefined && (
                    <div className="text-center p-3 bg-white dark:bg-gray-800 rounded-lg border border-blue-100 dark:border-blue-800">
                      <div className="text-xs text-blue-600 dark:text-blue-400 uppercase font-medium mb-1">Pd ppm</div>
                  ...
                  {product.rh_ppm !== undefined && (
                    <div className="text-center p-3 bg-white dark:bg-gray-800 rounded-lg border border-blue-100 dark:border-blue-800">
                      <div className="text-xs text-blue-600 dark:text-blue-400 uppercase font-medium mb-1">Rh ppm</div>
                ...
                <div className="mt-4 text-xs text-blue-600 dark:text-blue-400">
                  ...
                  These values are used for basket calculations and are only visible to Admin and Admin-Manager users.
                </div>
              </div>
            )}
...
function CatalystCalculationsSection({ weight, ptPpm, pdPpm, rhPpm }) {
  const [priceSettings, setPriceSettings] = useState(null);

  useEffect(() => {
    const fetchPriceSettings = async () => {
      try {
        const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/admin/price-settings`);
        if (response.ok) {
  ...
  if (!priceSettings || (!weight && !ptPpm && !pdPpm && !rhPpm)) {
    return null;
  }

  // Calculate g values using formula: weight * ppm / 1000 * renumeration
  const ptG = weight && ptPpm ? (weight * ptPpm / 1000) * (priceSettings.renumeration_pt || 0.98) : 0;
  const pdG = weight && pdPpm ? (weight * pdPpm / 1000) * (priceSettings.renumeration_pd || 0.98) : 0;
  const rhG = weight && rhPpm ? (weight * rhPpm / 1000) * (priceSettings.renumeration_rh || 0.9) : 0;

  return (
    <div className="bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20 rounded-xl p-6 border border-green-200 dark:border-green-800">
      <div className="flex items-center space-x-2 mb-4">
        <TrendingUp className="w-5 h-5 text-green-600 dark:text-green-400" />
        <h3 className="text-lg font-semibold text-green-900 dark:text-green-100">Catalyst Calculations</h3>
        <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-orange-100 text-orange-800 dark:bg-orange-900 dark:text-orange-200">
          Admin Only
        </span>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="text-center p-4 bg-white dark:bg-gray-800 rounded-lg border border-green-100 dark:border-green-800">
          <div className="text-xs text-green-600 dark:text-green-400 uppercase font-medium mb-1">Pt g</div>
          <div className="text-xl font-bold text-gray-900 dark:text-white">{ptG.toFixed(4)}</div>
        </div>
        
        <div className="text-center p-4 bg-white dark:bg-gray-800 rounded-lg border border-green-100 dark:border-green-800">
          <div className="text-xs text-green-600 dark:text-green-400 uppercase font-medium mb-1">Pd g</div>
          <div className="text-xl font-bold text-gray-900 dark:text-white">{pdG.toFixed(4)}</div>
        </div>
        
        <div className="text-center p-4 bg-white dark:bg-gray-800 rounded-lg border border-green-100 dark:border-green-800">
          <div className="text-xs text-green-600 dark:text-green-400 uppercase font-medium mb-1">Rh g</div>
          <div className="text-xl font-bold text-gray-900 dark:text-white">{rhG.toFixed(4)}</div>
        </div>
      </div>
      
      <div className="mt-4 text-xs text-green-600 dark:text-green-400">
        <Database className="w-4 h-4 inline mr-1" />
        Calculated using: Weight × ppm ÷ 1000 × Renumeration Factor
      </div>
    </div>
  );
}
