...
class NotificationDebugTester:
    ...
    def test_notification_workflow(self):
        ...
        if isinstance(self.notifications, list):
            initial_unread_count = len([n for n in self.notifications if isinstance(n, dict) and not n.get('is_read', True)])
        ...
        if self.test_mark_all_notifications_read():
            ...
            self.test_get_notifications()
            unread_notifications = [n for n in self.notifications if not n.get('is_read', True)]
