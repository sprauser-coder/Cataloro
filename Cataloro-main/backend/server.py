...
class VisitorAnalytics(BaseModel):
    ...
    visit_duration: Optional[int] = None  # in seconds
    actions_count: int = Field(default=0)  # clicks, searches, etc.
    device_type: Optional[str] = None  # mobile, desktop, tablet
    browser: Optional[str] = None
    location: Optional[str] = None  # country/city if available
...
class PageView(BaseModel):
    ...
    actions_taken: List[str] = Field(default_factory=list)  # ['search', 'click_listing', 'add_favorite']
...
class ConversionEvent(BaseModel):
    ...
    session_id: str
    user_id: str
    event_type: str  # 'registration', 'first_listing', 'first_purchase', 'first_sale'
...
@api_router.get("/")
async def root():
    return {"message": "Marketplace API", "version": "v1.6.5"}
...
async def remove_favorite(favorite_id: str, current_user: User = Depends(get_current_user)):
    """Remove item from favorites and track activity"""
    try:
        ...
        # Get listing details for activity tracking
        listing = await db.listings.find_one({"id": favorite["listing_id"]})
        ...
        if listing:
            await db.user_activities.insert_one(prepare_for_mongo({
                "id": str(uuid.uuid4()),
                "user_id": current_user.id,
                "activity_type": "item_unfavorited",
                "title": f"Removed from favorites: {listing['title']}",
...
async def delete_listing_with_activity(listing_id: str, current_user: User = Depends(get_current_user)):
    """Delete listing and track activity"""
    try:
        ...
        await db.user_activities.insert_one(prepare_for_mongo({
            ...
            "title": f"Deleted listing: {listing['title']}",
            "description": f"Was listed for €{listing['price']}",
            "timestamp": datetime.now(timezone.utc),
            "icon": "🗑️",
            "color": "red"
        }))
...
@api_router.post("/admin/fix-admin-name")
async def fix_admin_name(current_user: User = Depends(get_admin_user)):
    """Fix admin user display name if it shows 'Updated Admin Name' or similar issues"""
    try:
        # Update admin user to ensure proper full_name
        admin_updates = {
        ...
        if result.modified_count > 0:
            ...
            return {
                "message": "Admin name was already correct",
                "admin_name": current_user.full_name or "System Administrator", 
                "updated": False
            }
            
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to fix admin name: {str(e)}")
...
async def upload_logo(
    ...
    # Use environment variable or fallback to relative path for flexibility
    base_url = os.environ.get('BACKEND_BASE_URL', '')
    if base_url:
        logo_url = f"{base_url}/api/uploads/{unique_filename}"
    else:
        logo_url = f"/api/uploads/{unique_filename}"  # Relative path for local dev
    ...
    if logo_type == "header":
        ...
        if old_logo_url and old_logo_url.startswith(f"{base_url}/api/uploads/"):
...
async def upload_hero_image(
    ...
    # Generate URL
    base_url = os.environ.get('BACKEND_BASE_URL', 'https://cataloro-rebuild.preview.emergentagent.com')
...
async def get_user_stats(current_user: User = Depends(get_current_user)):
    ...
    earnings_pipeline = [
        {"$match": {"seller_id": current_user.id, "status": "completed"}},
...
@api_router.post("/analytics/track-visit")
async def track_visit(request: Request, visit_data: dict):
    """Track a page visit"""
    try:
        # Extract request info
        client_ip = await get_client_ip(request)
        user_agent = request.headers.get("User-Agent", "")
        user_info = parse_user_agent(user_agent)
        ...
        visit_record = {
            ...
            "page_visited": visit_data.get("page_visited", "/"),
        ...
        analytics_doc = prepare_for_mongo(visit_record)
        analytics_doc["id"] = str(uuid.uuid4())
        
        await db.visitor_analytics.insert_one(analytics_doc)
        
        return {"status": "success", "message": "Visit tracked"}
        
    except Exception as e:
        return {"status": "error", "message": f"Failed to track visit: {str(e)}"}

@api_router.post("/analytics/track-action")
async def track_action(action_data: dict):
    """Track user actions like searches, clicks, favorites"""
    try:
        # Update the session with new action
        await db.visitor_analytics.update_one(
        ...
        return {"status": "error", "message": f"Failed to track action: {str(e)}"}

@api_router.get("/admin/analytics/dashboard")
async def get_analytics_dashboard(current_user: User = Depends(get_admin_user)):
    ...
    try:
        ...
        # Today's stats
        today_visits = await db.visitor_analytics.count_documents({
            "visited_at": {"$gte": today_start}
        })
        today_unique = len(await db.visitor_analytics.distinct("ip_address", {
            "visited_at": {"$gte": today_start}
        }))
        
        # Weekly stats
        week_visits = await db.visitor_analytics.count_documents({
            "visited_at": {"$gte": week_start}
        })
        week_unique = len(await db.visitor_analytics.distinct("ip_address", {
            "visited_at": {"$gte": week_start}
        }))
        ...
        device_stats = await db.visitor_analytics.aggregate(device_pipeline).to_list(None)
        ...
        browser_stats = await db.visitor_analytics.aggregate(browser_pipeline).to_list(None)
        
        # Top pages
        pages_pipeline = [
            {"$group": {"_id": "$page_visited", "visits": {"$sum": 1}}},
            {"$sort": {"visits": -1}},
            {"$limit": 10}
        ]
        top_pages = await db.visitor_analytics.aggregate(pages_pipeline).to_list(None)
        
        # Conversion rate (visitors who became users)
        total_users = await db.users.count_documents({})
        conversion_rate = (total_users / max(unique_visitors, 1)) * 100
        
        # Average session duration (mock for now - would need session end tracking)
        avg_session_duration = 180  # 3 minutes average
        
        # Bounce rate (visitors with only 1 action)
        bounce_visitors = await db.visitor_analytics.count_documents({
            "actions_count": {"$lte": 1}
        })
        bounce_rate = (bounce_visitors / max(total_visits, 1)) * 100
        
        # Popular search terms (if we track searches)
        search_pipeline = [
            {"$match": {"actions.action": "search"}},
            {"$unwind": "$actions"},
            {"$match": {"actions.action": "search"}},
            {"$group": {"_id": "$actions.data.query", "count": {"$sum": 1}}},
            {"$sort": {"count": -1}},
            {"$limit": 10}
        ]
        popular_searches = await db.visitor_analytics.aggregate(search_pipeline).to_list(None)
        ...
        return {
            ...
            "growth_metrics": {
                "daily_visitors": today_unique,
                "weekly_visitors": week_unique,
...
async def get_order_details(order_id: str, current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        # Get buyer details
        buyer = None
        if order_doc.get("buyer_id"):
            buyer_doc = await db.users.find_one({"id": order_doc["buyer_id"]})
            if buyer_doc:
                buyer = {
                    "id": buyer_doc["id"],
                    "username": buyer_doc.get("username", "Unknown"),
                    "full_name": buyer_doc.get("full_name", ""),
                    "email": buyer_doc.get("email", "")
                }
        
        # Get seller details
        seller = None
        if order_doc.get("seller_id"):
            seller_doc = await db.users.find_one({"id": order_doc["seller_id"]})
            if seller_doc:
                seller = {
                    "id": seller_doc["id"],
                    "username": seller_doc.get("username", "Unknown"),
                    "full_name": seller_doc.get("full_name", ""),
        ...
        order_details = {
            ...
            "created_at": order_doc["created_at"].isoformat() if hasattr(order_doc["created_at"], 'isoformat') else str(order_doc["created_at"]),
            "updated_at": order_doc.get("updated_at", "").isoformat() if hasattr(order_doc.get("updated_at", ""), 'isoformat') else str(order_doc.get("updated_at", "")),
            "listing": listing,
            "buyer": buyer,
            "seller": seller,
            ...
            "transaction_fee": order_doc.get("transaction_fee", 0),
            "is_buyer": order_doc["buyer_id"] == current_user.id,
            "is_seller": order_doc["seller_id"] == current_user.id
        }
...
async def get_public_profile(user_id: str):
    """Get public profile information (no sensitive data)"""
    try:
        # Get user basic info
        ...
        # Calculate completed orders as seller
        completed_orders = await db.orders.count_documents({"seller_id": user_id, "status": "completed"})
        
        # Public profile data (no sensitive information)
        public_profile = {
            "user_id": user_id,
...
@api_router.get("/public-profile/{user_id}/listings")
async def get_public_user_listings(user_id: str, status: str = "active"):
    """Get public user's listings"""
    try:
        # Verify user exists
        user_doc = await db.users.find_one({"id": user_id})
        if not user_doc:
        ...
        listings = await db.listings.find(query).sort("created_at", -1).to_list(None)
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get public listings: {str(e)}")
