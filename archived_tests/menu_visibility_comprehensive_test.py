...
class MenuSettingsVisibilityTester:
    ...
    async def test_admin_menu_settings_update(self) -> Dict:
        ...
        print(f"    📋 Desktop menu items: {list(current_settings.get('desktop_menu', {}).keys())}")
        print(f"    📱 Mobile menu items: {list(current_settings.get('mobile_menu', {}).keys())}")
        ...
        if "desktop_menu" in modified_settings:
            for item_key in modified_settings["desktop_menu"]:
                if "message" in item_key.lower():
                    modified_settings["desktop_menu"][item_key]["enabled"] = False
                    messages_disabled = True
                    print(f"    📱 Disabled messages in desktop menu: {item_key}")
        
        if "mobile_menu" in modified_settings:
            for item_key in modified_settings["mobile_menu"]:
                if "message" in item_key.lower():
                    modified_settings["mobile_menu"][item_key]["enabled"] = False
                    messages_disabled = True
                    print(f"    📱 Disabled messages in mobile menu: {item_key}")
        ...
        if not messages_disabled:
            return {
                ...
                "current_settings_structure": {
                    "desktop_keys": list(current_settings.get("desktop_menu", {}).keys()),
                    "mobile_keys": list(current_settings.get("mobile_menu", {}).keys())
                }
            }
        ...
        for menu_type in ["desktop_menu", "mobile_menu"]:
            if menu_type in saved_settings:
                for item_key, item_config in saved_settings[menu_type].items():
                    if "message" in item_key.lower() and not item_config.get("enabled", True):
                        messages_disabled_in_saved = True
                        disabled_items.append(f"{menu_type}:{item_key}")
    ...
    async def test_user_menu_settings_retrieval(self) -> Dict:
        ...
        if admin_menu_result["success"]:
            ...
            test_results["admin_user"] = {
                ...
                "user_role": admin_menu_data.get("user_role", "unknown")
            }
            
            print(f"    ✅ Admin menu settings retrieved - Messages visible: {messages_visible_admin}")
            print(f"    📋 Desktop items: {len(admin_menu_data.get('desktop_menu', {}))}, Mobile items: {len(admin_menu_data.get('mobile_menu', {}))}")
        else:
            test_results["admin_user"] = {
        ...
        if demo_menu_result["success"]:
            ...
            test_results["demo_user"] = {
                ...
                "user_role": demo_menu_data.get("user_role", "unknown")
            }
            
            print(f"    ✅ Demo menu settings retrieved - Messages visible: {messages_visible_demo}")
            print(f"    📋 Desktop items: {len(demo_menu_data.get('desktop_menu', {}))}, Mobile items: {len(demo_menu_data.get('mobile_menu', {}))}")
        else:
            test_results["demo_user"] = {
                "endpoint_accessible": False,
        ...
        if admin_settings_result["success"]:
            ...
            for menu_type in ["desktop_menu", "mobile_menu"]:
                if menu_type in admin_settings:
                    for item_key, item_config in admin_settings[menu_type].items():
                        if "message" in item_key.lower() and not item_config.get("enabled", True):
    ...
    def check_messages_visibility(self, menu_data: Dict) -> bool:
        ...
        for menu_type in ["desktop_menu", "mobile_menu"]:
            if menu_type in menu_data:
                for item_key, item_config in menu_data[menu_type].items():
                    if "message" in item_key.lower():
                        # Check if item is enabled (visible)
                        return item_config.get("enabled", True)
    ...
    def analyze_menu_structure(self, menu_data: Dict) -> Dict:
        ...
        desktop_items = len(menu_data.get("desktop_menu", {}))
        mobile_items = len(menu_data.get("mobile_menu", {}))
    ...
    def extract_menu_keys(self, menu_data: Dict) -> Dict:
        ...
        if "desktop_menu" in menu_data:
            for item_key in menu_data["desktop_menu"]:
                desktop_keys.append(item_key.lower())
                all_keys.append(item_key.lower())
        
        if "mobile_menu" in menu_data:
            for item_key in menu_data["mobile_menu"]:
                mobile_keys.append(item_key.lower())
                if item_key.lower() not in all_keys:
                    all_keys.append(item_key.lower())
...
async def main():
    ...
    with open('/app/menu_visibility_test_results.json', 'w') as f:
        json.dump(results, f, indent=2, default=str)
    
    print(f"\n📄 Detailed results saved to menu_visibility_test_results.json")
